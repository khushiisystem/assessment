...
const ActiveSession: React.FC = () => {
  ...
  const loadSession = async () => {
    setIsLoading(true);
    try {
      const [sessionData, questionsResult] = await Promise.all([
        getSessionById(Number(sessionId)).unwrap(),
        getMockQuestions().unwrap(),
      ]);
      const allQuestions = questionsResult?.results ?? questionsResult;
  ...
  return (
    <AdminLayout>
          ...
          {currentQuestion && (
            <Card className="border border-gray-200 shadow-sm mb-4">
              ...
              <CardContent className="pt-0 space-y-4">
                ...
                {/* Notes */}
                <div>
                  <Label className="text-xs font-medium text-slate-700 mb-2 block">Feedback</Label>
                  <Textarea
                    placeholder="Add feedback about the candidate's response..."
      ...
      <ConfirmationDialog
        ...
        title="End Interview"
        description="Are you sure you want to end the interview? Your responses will be saved automatically and you will be redirected to the finish page."
